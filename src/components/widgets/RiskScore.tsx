  ...
  return (
    <Card className="glass p-4 md:p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-display text-sm font-semibold">리스크 점수</h3>
      </div>
      <div className="flex items-end gap-2">
        <span className={`font-display text-2xl md:text-3xl font-semibold leading-none ${color}`}>
          {score}
        </span>
        <span className="text-xs text-muted-foreground mb-0.5">/ 100</span>
        <span className={`text-xs font-medium ${color} ml-auto`}>{level}</span>
      </div>
